...
------ MULU/MULS
---------------------------------------------------------------------------------	
	-- Keep the hardware and iterative implementations in separate static
	-- generate branches.  Besides making the intended resource choice explicit,
	-- this prevents older synthesis front ends from elaborating the dormant
	-- 64x64 multiplier when MUL_Hardware=0.
	gen_mul_hardware : IF MUL_Hardware=1 GENERATE
		PROCESS (exe_opcode, OP2out, reg_QA, signedOP)
			VARIABLE factor_a_v : std_logic_vector(31 downto 0);
			VARIABLE factor_b_v : std_logic_vector(31 downto 0);
			VARIABLE result_v   : std_logic_vector(127 downto 0);
		BEGIN
			factor_a_v := (OTHERS=>'0');
			factor_b_v := (OTHERS=>'0');
			result_v := (OTHERS=>'0');
			...
			IF MUL_Mode=0 THEN	-- 16 Bit
				IF signedOP='1' AND reg_QA(15)='1' THEN
					factor_a_v := X"FFFFFFFF";
				END IF;
				IF signedOP='1' AND OP2out(15)='1' THEN
					factor_b_v := X"FFFFFFFF";
				END IF;
				result_v(63 downto 0) :=
					(factor_a_v(15 downto 0) & reg_QA(15 downto 0)) *
					(factor_b_v(15 downto 0) & OP2out(15 downto 0));
			ELSE
				IF exe_opcode(15)='1' THEN	-- 16 Bit
					...
					END IF;
				ELSE
					factor_a_v(15 downto 0) := reg_QA(31 downto 16);
					factor_b_v(15 downto 0) := OP2out(31 downto 16);
					IF signedOP='1' AND reg_QA(31)='1' THEN
						factor_a_v(31 downto 16) := X"FFFF";
					END IF;
					IF signedOP='1' AND OP2out(31)='1' THEN
						factor_b_v(31 downto 16) := X"FFFF";
					END IF;
				END IF;
				result_v :=
					(factor_a_v(31 downto 16) & factor_a_v & reg_QA(15 downto 0)) *
					(factor_b_v(31 downto 16) & factor_b_v & OP2out(15 downto 0));
			END IF;
			result_mulu <= result_v;
		END PROCESS;
		...
		BEGIN
			...
			END IF;
		END PROCESS;
	END GENERATE gen_mul_hardware;

	gen_mul_iterative : IF MUL_Hardware=0 GENERATE
		PROCESS (exe_opcode, OP2out, mulu_reg, FAsign, signedOP)
			VARIABLE factor_b_v  : std_logic_vector(31 downto 0);
			VARIABLE muls_msb_v  : std_logic;
			VARIABLE mulu_sign_v : std_logic;
			VARIABLE result_v    : std_logic_vector(127 downto 0);
		BEGIN
			IF exe_opcode(15)='1' OR MUL_Mode=0 THEN
				factor_b_v := OP2out(15 downto 0) & X"0000";
			ELSE
				factor_b_v := OP2out;
			END IF;

			mulu_sign_v := signedOP AND factor_b_v(31);
			IF mulu_sign_v='1' OR FAsign='1' THEN
				muls_msb_v := mulu_reg(63);
			ELSE
				muls_msb_v := '0';
			END IF;

			-- Fully assign the result in one delta cycle.  The upstream process
			-- reached this same stable value through self-sensitive helper signals.
			result_v := (OTHERS=>'0');
			IF MUL_Mode=0 THEN	-- 16 Bit
				result_v(63 downto 32) := muls_msb_v & mulu_reg(63 downto 33);
				result_v(15 downto 0) := 'X' & mulu_reg(15 downto 1);
				IF mulu_reg(0)='1' THEN
					IF FAsign='1' THEN
						result_v(63 downto 47) :=
							(muls_msb_v & mulu_reg(63 downto 48)) -
							(mulu_sign_v & factor_b_v(31 downto 16));
					ELSE
						result_v(63 downto 47) :=
							(muls_msb_v & mulu_reg(63 downto 48)) +
							(mulu_sign_v & factor_b_v(31 downto 16));
				...
				END IF;
			ELSE				-- 32 Bit
				result_v(63 downto 0) := muls_msb_v & mulu_reg(63 downto 1);
				IF mulu_reg(0)='1' THEN
					IF FAsign='1' THEN
						result_v(63 downto 31) :=
							(muls_msb_v & mulu_reg(63 downto 32)) -
							(mulu_sign_v & factor_b_v);
					ELSE
						result_v(63 downto 31) :=
							(muls_msb_v & mulu_reg(63 downto 32)) +
							(mulu_sign_v & factor_b_v);
		...
		BEGIN
			...
			END IF;
		END PROCESS;
	END GENERATE gen_mul_iterative;

	PROCESS (result_mulu, signedOP)
	BEGIN
